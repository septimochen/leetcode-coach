...
def _problem_data(problems: list[Problem]) -> list[dict[str, object]]:
    return [{"title": p.title, "lc_id": p.frontend_id, "slug": p.title_slug, "difficulty": p.difficulty, "topics": p.topic_tags, "url": p.url} for p in problems]
...
def _sample(problems: list[Problem], maximum: int) -> list[Problem]:
    ...
    if len(problems) <= maximum:
        return problems
    buckets = [[p for p in problems if p.difficulty == difficulty] for difficulty in ("Easy", "Medium", "Hard")]
...
def create_weekly_plan(*, solved: list[Problem], catalog: list[Problem], model: str, api_key: str, base_url: str | None = None, start_day: date | None = None) -> WeeklyPlan:
    ...
    prompt = json.dumps({"week_start": start_day.isoformat(), "solved_problems": _problem_data(_sample(solved, 100)), "unsolved_problems": _problem_data(_sample(unsolved, 150))}, ensure_ascii=False)
    ...
    try:
        ...
        raise RuntimeError("The model returned an invalid study plan; verify JSON-mode support for the configured provider.") from error
    ...
    plan = WeeklyPlan(learner_summary=draft.learner_summary, strengths=draft.strengths, growth_areas=draft.growth_areas, days=[DayPlan(day=entry.day, focus=entry.focus, review=enrich(entry.review, solved), practice=enrich(entry.practice, unsolved), rationale=entry.rationale) for entry in draft.days])
    expected_days = [start_day + timedelta(days=index) for index in range(7)]
    if [entry.day for entry in plan.days] != expected_days:
        raise RuntimeError("The model returned dates outside the requested seven-day window.")
    return plan
